...
export class Frog {
  ...
  tilePosition(): { x: number; y: number } {
    return { x: this.col * TILE + this.driftX, y: this.row * TILE };
  }
  ...
  pixelPosition(): { x: number; y: number; t: number } {
    if (!this.hopFrom) {
      return { x: this.col * TILE + this.driftX, y: this.row * TILE, t: 0 };
    }
    const t = Math.min(1, this.hopT / HOP_DURATION);
    const fromX = this.hopFrom.col * TILE;
    const fromY = this.hopFrom.row * TILE;
    const toX = this.col * TILE;
    const toY = this.row * TILE;
    return {
      x: fromX + (toX - fromX) * t,
      y: fromY + (toY - fromY) * t,
      t,
